...
extension Variable {
    convenience init(
      ...
      isStatic: Bool,
      attributesFromModifiers: [Attribute],
      visitingType: Type?,
      annotationParser: AnnotationsParser
        ...
        self.init(
          ...
          attributes: Attribute.from(variableNode.attributes, adding: attributesFromModifiers),
    ...
    static func from(_ variableNode: VariableDeclSyntax, visitingType: Type?,
        ...
        return variableNode.bindings.map { (node: PatternBindingSyntax) -> Variable in
            Variable(
              ...
              attributesFromModifiers: modifiers.map(Attribute.init),
    ...
    private static func inferType(_ code: String) -> TypeName? {
        ...
        let inferred = FileParserSyntax.inferType(from: code)
        return inferred.map { TypeName($0) } ?? nil
    }
}
